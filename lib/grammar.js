...
import { o, eat, eatMatch, exec, fail, match, defineAttribute } from '@bablr/helpers/grammar';
...
export const defaultMatcher = m`<_MatcherExpression />`;
...
export const grammar = class SpamexGrammar {
  ...
  *ReferenceMatcher() {
    ...
    if (yield eatMatch(m`openIndexToken*: <* '[' />`)) {
      yield* eatMatchTrivia();
      yield eatMatch(m`closeIndexToken*: <* ']' />`);
    }
